...
stock BlockGarages(bool:block = true, GarageType:gar_type = GARAGE_TYPE_ALL, text[] = "")
{
	if(gar_type == GARAGE_TYPE_ALL) {
		if(block == false) {
		...
		if(block == false) {
			for(new i; i < sizeof(G_Blocker); i++) {
				if(G_Blocker[i][g_type] == gar_type)
					DestroyDynamicObject(G_Blocker[i][g_object]);
			}
		}
		else {
			for(new i; i < sizeof(G_Blocker); i++) {
				if(G_Blocker[i][g_type] == gar_type) {
					...
					if(!isnull(text))
						SetDynamicObjectMaterialText(G_Blocker[i][g_object], 0, text, OBJECT_MATERIAL_SIZE_128x128, "Arial", 12, 0, 0xFFFF6600, 0x00000000, OBJECT_MATERIAL_TEXT_ALIGN_CENTER);
					else
						SetDynamicObjectMaterial(G_Blocker[i][g_object], 0, 0, "none", "none", 0x00FFFFFF);
				}
			}
...
hook OnGameModeInit() {
	BlockGarages(true, GARAGE_TYPE_PAINT, "(( BLOCKED ))");
	BlockGarages(true, GARAGE_TYPE_BOMB, "(( BLOCKED ))");
	BlockGarages(true, GARAGE_TYPE_MODSHOP, "(( BLOCKED ))");
